...
document.querySelector('#account-info').addEventListener('click', () => showAlert({ title: 'Neste tekniske fase', html: '<p>Produksjonsversjonen skal få Logg inn med Apple, skylagring, flere hjem og ekte invitasjonslenker. Prototypen lagrer fortsatt på denne enheten.</p>' }));
...
document.querySelector('#help-button').addEventListener('click', () => showAlert({ title: 'Slik bruker du 1.0', html: '<ol><li>Opprett rom og plasseringer som skap, skuffer og kasser.</li><li>Legg ting i riktig plassering og bruk korte tagger.</li><li>Åpne en plassering for å vise eller skrive ut QR-etiketten.</li><li>Flytt en ting ved å åpne den og velge «Flytt eller rediger».</li></ol>' }));
document.querySelector('#about-button').addEventListener('click', () => showAlert({ title: 'Hvor er den? 1.0', html: `<p>En felles hjemmeapp for ting, rom, kasser og skap.</p><p><strong>${state.data.items.length}</strong> ting og <strong>${state.data.containers.length}</strong> plasseringer ligger i ${escapeHtml(state.data.home.name)}.</p>` }));
